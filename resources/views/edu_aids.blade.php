  ...
  <script>
    function del(id, token) {
      if (confirm("Вы действительно хотите удалить материал ID \"" + id + "\"?")) {
          location.href="/admin/edu_aids/" + id +"/delete?token=" + token ;   
      }
    }
  </script>
</head>
...
<body>
  <main style="display: flex; flex-direction: row;">
    <div class="form" style="display: flex; flex-direction: column; width:25%;">
      <form action="/admin/edu_aids/sort?token={{ $edu_aids->token }}" method="post" style="display: flex; flex-direction: column;">
        @csrf
        <select name="field">
      ...
      <form action="/admin/edu_aids/search?token={{ $edu_aids->token }}" method="post" style="display: flex; flex-direction: column;">
        @csrf
      ...
      <form action="/admin/edu_aids/create?token={{ $edu_aids->token }}" enctype="multipart/form-data" method="post" style="display: flex; flex-direction: column;">
        @csrf
      ...
      ?>
      <a href="/admin?token={{ $edu_aids->token }}"><button>Назад</button></a>
    </div>
    <table class="table">        
      <thead>
        <?php
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><a href="/admin/edu_aids/{{ $element->id }}/update?token={{ $edu_aids->token }}">Редактировать</a></td>
          <td><a href="javascript:del({{ $element->id }}, '{{ $edu_aids->token }}')">Удалить</a></td>
        </tr>
        @endforeach
      </tbody>              
